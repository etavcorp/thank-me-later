...
function getAllowedOrigins(env) {
  const configuredOrigins = (env.ALLOWED_ORIGINS ?? "").split(",").map((origin) => origin.trim()).filter(Boolean);
  return [.../* @__PURE__ */ new Set([...DEFAULT_ALLOWED_ORIGINS, ...configuredOrigins])];
}
...
function getRequestOrigin(request, env) {
  ...
  try {
    ...
    const isAllowedHost = hostname === "thankmelatercatering.com" || hostname.endsWith(".thankmelatercatering.com") || hostname.endsWith(".etavcorp.workers.dev") || hostname === "etavcorp.workers.dev";
...
__name(getBodyJson, "getBodyJson");
...
async function signJwtLike(payload, secret) {
...
__name(signJwtLike, "signJwtLike");
async function verifyJwtLike(token, secret) {
...
__name(verifyJwtLike, "verifyJwtLike");
...
async function generateTotp(secret, timeStepSeconds) {
...
async function verifyTotp(secret, candidateCode) {
  if (!secret || !candidateCode || !/^\d{6}$/.test(candidateCode.trim())) {
    return false;
  }
  const code = candidateCode.trim();
  const timeStepSeconds = Math.floor(Date.now() / 1e3);
...
function createSessionToken(user, env) {
...
async function requireAuth(request, env, allowedRoles = ["admin", "editor", "viewer"]) {
...
async function createTrustedDeviceToken(userId, env) {
...
__name(createTrustedDeviceToken, "createTrustedDeviceToken");
async function verifyTrustedDeviceToken(userId, token, env) {
  if (!token) {
    return false;
  }
  const tokenHash = await hashToken(token);
...
__name(verifyTrustedDeviceToken, "verifyTrustedDeviceToken");
async function ensureDbSchema(env) {
  ...
  if (userTableInfo.results === void 0 || userTableInfo.results.length === 0) {
    await env.DB.prepare(`
      CREATE TABLE users (
...
__name(ensureDbSchema, "ensureDbSchema");
async function seedDefaultAdminIfEmpty(env) {
  if (!env.DB) {
    throw new Error("D1 database binding is missing.");
  }
...
__name(seedDefaultAdminIfEmpty, "seedDefaultAdminIfEmpty");
async function seedMenuItemsIfEmpty(env) {
  if (!env.DB) {
    throw new Error("D1 database binding is missing.");
  }
  const countResult = await env.DB.prepare("SELECT COUNT(*) AS count FROM menu_items").first();
...
__name(seedMenuItemsIfEmpty, "seedMenuItemsIfEmpty");
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/auth/login" && request.method === "POST") {
        const body = await getBodyJson(request);
        ...
        let trustedDeviceTokenResponse = null;
      ...
      if (url.pathname === "/api/auth/me" && request.method === "PATCH") {
        ...
        let body = {};
        try {
          body = await getBodyJson(request);
        } catch {
          body = {};
        }
        const username = typeof body.username === "string" ? body.username.trim() : "";
        ...
        const rawTotpSecret = typeof body.totpSecret === "string" ? body.totpSecret.trim() : typeof body.totp_secret === "string" ? body.totp_secret.trim() : "";
        if (!username) {
        ...
        ).bind(username, auth.user.id).first();
        if (existingUser) {
      ...
      if ((url.pathname === "/api/auth/setup-admin" || url.pathname === "/api/admin/bootstrap" || url.pathname === "/api/admin/register") && request.method === "POST") {
        ...
        const username = typeof body.username === "string" ? body.username.trim() : typeof body.email === "string" ? body.email.trim() : typeof body.name === "string" ? body.name.trim() : "";
        const password = typeof body.password === "string" ? body.password : "";
        const role = typeof body.role === "string" && ["admin", "editor", "viewer"].includes(body.role) ? body.role : "admin";
        const rawTotpEnabled = body.totpEnabled ?? body.totp_enabled ?? body.enableTotp ?? body.enable_totp ?? false;
        const totpEnabled = rawTotpEnabled === true || rawTotpEnabled === "true" || rawTotpEnabled === 1 || rawTotpEnabled === "1" ? 1 : 0;
        const rawTotpSecret = typeof body.totpSecret === "string" ? body.totpSecret : typeof body.totp_secret === "string" ? body.totp_secret : "";
        const totpSecret = rawTotpSecret.trim() ? rawTotpSecret.trim() : null;
        const totalsCount = await env.DB.prepare("SELECT COUNT(*) AS count FROM users").first();
      ...
      if (url.pathname === "/api/bookings" && request.method === "POST") {
        ...
        const result = await env.DB.prepare(
          "INSERT INTO bookings (name, phone, email, type, guests, date, notes, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'New', CURRENT_TIMESTAMP)"
        ).bind(name, phone, email, type, String(guests), date, notes).run();
        return jsonResponse(
          {
            id: result.meta.last_row_id ?? null,
            name,
      ...
      if (url.pathname.startsWith("/api/admin/bookings/") && request.method === "PUT") {
        ...
        const existing = await env.DB.prepare("SELECT id FROM bookings WHERE id = ? LIMIT 1").bind(id).first();
        if (!existing) {
      ...
      if (url.pathname === "/api/menu" && request.method === "POST") {
        ...
        ).bind(title, price, description).run();
        return jsonResponse(
          {
            id: result.meta.last_row_id ?? null,
            title,
      ...
      if (url.pathname.startsWith("/api/menu/") && request.method === "PUT") {
        ...
        ).bind(title, price, description, id).run();
